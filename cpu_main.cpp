...
void leaky_relu(float* A, int size, float alpha) {
    for (int i = 0; i < size; ++i) {
        float x = A[i];
        A[i] = (x > 0.0f) ? x : alpha * x;
    }
}
...
int main() {
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        ...
        leaky_relu(hidden.data(), hidden.size(), 0.01f);
    ...
    leaky_relu(hidden_test.data(), hidden_test.size(), 0.01f);
